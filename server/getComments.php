...
if(isset($_GET['ticket_id'])){
	...
	try {
		...
		$stmt = $dbh->prepare("SELECT c.id, c.text, c.file, c.date, u.role FROM Comment c join User u on c.user = u.id where ticket = :ticket_id ORDER BY c.id");
		$stmt->bindValue(':ticket_id', $ticket_id, PDO::PARAM_INT);
		$stmt->execute();
		$arr =[];
		$zone = $_SESSION['logged_user']['zone'];
		while ($row = $stmt->fetch()) {
		    array_push($arr, array('id' => $row['id'], 'text' => $row['text'], 'file' => $row['file'], 'date' => TimeService::dateFull($row['date'], ' в ', $zone), 'role' => $row['role']));
		}
